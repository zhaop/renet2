...
#[cfg(feature = "transport")]
use {
    renet2::transport::NetcodeClientTransport,
...
impl RepliconRenetServerPlugin {
    ...
    fn forward_server_events(mut renet_server_events: EventReader<renet2::ServerEvent>, mut server_events: EventWriter<ServerEvent>) {
        for event in renet_server_events.read() {
            let replicon_event = match event {
                renet2::ServerEvent::ClientConnected { client_id } => ServerEvent::ClientConnected {
                    client_id: ClientId::new(client_id.raw()),
                },
                renet2::ServerEvent::ClientDisconnected { client_id, reason } => ServerEvent::ClientDisconnected {
                    client_id: ClientId::new(client_id.raw()),
